...
import { createIndustry } from '@/app/actions/generics';
...
function SubmitButton({ isEditing }: { isEditing: boolean }) {
  ...
  return (
    ...
    >
      ...
      />{' '}
      <span>{isEditing ? 'Edit' : 'Create'} Industry</span>
    </Button>
  );
}
...
export function IndustryModal({
  ...
  const [state, dispatch] = useFormState(createIndustry, {
    ...
    >
      <DialogTrigger asChild>
        {isEditing ? (
          ...
          <Button size={'xl'}>Create Industry</Button>
        )}
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className='text-foreground text-xl'>
            {isEditing ? 'Edit' : 'Create'} Industry
          </DialogTitle>
        </DialogHeader>
